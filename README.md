...
**Querying an indexed Korean PDF** — 라우터 → 하이브리드 검색 → BGE 리랭킹 → vLLM SSE 토큰 스트림.

![query demo](docs/demo-query.gif)

**End-to-end pipeline** — PDF 업로드 → OCR + 임베딩 + 그래프 추출 → 쿼리 → SSE 응답.

![e2e demo](docs/demo-e2e.gif)
...
| 8 | Graph 품질 수정 — 토큰 경계 시드 매칭 (Fix A) + rerank 융합 (Fix B), 플래그 게이트 | DONE — `context_precision` 0.350→0.815 |

---

## 평가 결과 — build → measure → diagnose → fix → re-measure

> **요약 (takeaway)**: 본 평가는 그래프 시드 매칭의 약점(짧은 한국어 엔티티의 **부분 문자열
> 오탐**)을 정량적으로 **실증**하고, 근본 원인을 진단한 뒤 **토큰 경계 매칭 + 리랭커 기반
> 융합**으로 **수정·재측정**한 완결된 루프의 결과다. 핵심 지표 `context_precision`이
> graph+vector 기준 **0.350 → 0.815 로 회복**되어 vector-only와 **동률**에 도달했다. (이
> N=12 단일 단어 엔티티 합성셋에서 graph가 vector를 *능가*하지는 않는다 — 정직히 동률이며,
> graph의 이점은 대규모·다중 홉 코퍼스에서 드러난다.)

`eval/run_eval.py`로 실행한 합성 평가. **가상 공공기관(KPRI) 출장정책 도메인의 한국어
PNG 4개**에 **12 골든 질문**(단일 사실 5 + 멀티홉 5 + 함정 2)을 두 모드로 실행했다:
**GraphRAG (graph + vector)** vs **vector-only**. 동일 retrieve 파이프라인 + 동일 RAG 프롬프트 +
동일 로컬 vLLM(Gemma 4 26B AWQ-4bit)이 답변·judge 양쪽에 사용된다. 아래 before/after는 **동일
세션·동일 코퍼스·동일 judge**에서 측정했고, 유일한 차이는 두 개의 env 플래그
(`GRAPH_SEED_USE_TOKEN_BOUNDARY`, `GRAPH_RERANK_FUSION`)뿐이다 — `eval/run_eval.py`는 정직성을
위해 수정하지 않았다.

**Before (Fix off = Phase 1 동작):**
...
**After (Fix on = 토큰 경계 시드 매칭 + rerank 융합):**
...
핵심 변화 — `context_precision` (graph+vector), 카테고리별:

| 카테고리 | before | after |
|---|---|---|
| single-fact | 0.407 | **0.983** |
| multi-hop | 0.433 | **0.973** |

전체 점수·카테고리 분해는 [`eval/results_before.md`](eval/results_before.md) /
[`eval/results_after.md`](eval/results_after.md), 원본 응답·소스 청크는
[`eval/raw_results_before.json`](eval/raw_results_before.json) /
[`eval/raw_results_after.json`](eval/raw_results_after.json).

**진단 (Phase 1)**: `graph_search`가 `$question CONTAINS toLower(seed.name)` substring 매칭으로
시드를 잡고 `_merge`가 graph 히트를 응답 앞으로 무조건 prepend 하므로, "원장"(↔"부원장"),
"출장"(↔"출장비") 같은 짧은 한국어 엔티티가 question 안에 우연히 포함돼 false-positive 청크가
상위로 올라왔다. 이로 인해 `context_precision`이 0.815 → 0.350 으로 반토막 났다 — 본 레포 코드
주석에 적혀 있던 리스크 가설의 실증.

**수정 (Phase 2, 둘 다 기본 off 플래그)**:
- **Fix A — 토큰 경계 시드 매칭** (`GRAPH_SEED_USE_TOKEN_BOUNDARY`): `app/retrieve/tokenizer.py`가
  질문을 NFKC 정규화 + 한국어 조사 제거(누적 조사까지 fixpoint 반복) 후 토큰 집합으로 만들고,
  Cypher가 `toLower(seed.name) IN $tokens` 로 정확 일치를 요구한다. "출장"이 "출장비" 안에서
  매칭되는 substring 오탐이 사라진다.
- **Fix B — rerank 융합** (`GRAPH_RERANK_FUSION`): graph+vector 후보를 하나의 풀로 합쳐
  cross-encoder 점수로 정렬한다. graph 히트가 더 이상 무조건 앞자리를 차지하지 않고, 노이즈는
  리랭커가 걸러낸다.

**결과 해석 (정직성)**: 두 수정으로 `context_precision`이 0.350 → 0.815 로 완전히 회복되어
graph+vector가 vector-only와 **동률**에 도달했다(single-fact 0.407→0.983, multi-hop
0.433→0.973). 진단된 회귀는 제거됐다. 다만 이 작은 단일 단어 엔티티 합성셋에서는 graph가
vector-only를 **능가하지 않는다**(faithfulness 0.903 vs 0.944, latency 13.5s vs 12.2s) — graph
보강은 이 규모에서 *손해에서 동률로* 바뀌었을 뿐, 우위는 아니다. 이는 GraphRAG가 대규모·다중
홉 코퍼스에서 빛난다는 알려진 특성과 일치하며, 작은 합성셋의 결과를 graph 승리로 위조하지
않는다. control(vector-only)은 두 실행에서 사실상 불변(`context_precision` 0.815 = 0.815,
`context_recall` 0.778 = 0.778)이라 graph의 precision 점프가 judge drift가 아닌 수정의 효과임을
뒷받침한다.

**한계 (정직성)**:
- N=12 합성셋 · 작성자 1인 단독 라벨링이라 **방향성 sanity check** 이지 정밀 벤치가 아니다.
- generator 와 judge 가 같은 vLLM 을 공유하므로 절대 점수는 낙관적으로 편향된다 (양쪽 모두
  같은 hallucination 모드를 공유). 함정 카테고리(N=2) faithfulness 의 before/after 차이
  (1.000 → 0.750)는 이 소표본·공유 judge 노이즈 범위로, 집계 faithfulness 는 오히려 0.881 →
  0.903 으로 상승했다.
- 함정(trap-no-evidence) 질문 2개는 정의상 reference 가 없어 `context_recall` 과
  `context_precision` 이 0 / N/A 로 나타난다 (집계 표에는 0 으로 포함).
- 토큰 경계 모드는 공백을 포함한 다중 단어 엔티티명을 시드하지 못한다(이 합성셋은 단일 단어
  엔티티 위주라 무관). 기본값 off 이므로 Phase 1 동작에는 영향이 없다.
...
# BEFORE (Phase 1 동작 = 플래그 off)
docker exec llm-engine-app-1 python -m eval.run_eval

# AFTER (Fix A+B on — 단일 env 플래그 플립, run_eval.py 무수정)
docker exec -e GRAPH_SEED_USE_TOKEN_BOUNDARY=true -e GRAPH_RERANK_FUSION=true \
    llm-engine-app-1 python -m eval.run_eval
...
6. **Graph 검색 시드 정확도** — ✅ **Phase 2에서 수정** (플래그 게이트, 기본 off). 기본
   `graph_search`는 여전히 `toLower(seed.name)` substring 매칭이라 짧은 엔티티에서 false
   positive가 발생하지만, `GRAPH_SEED_USE_TOKEN_BOUNDARY=true`로 토큰 경계 정확 매칭을,
   `GRAPH_RERANK_FUSION=true`로 prepend 대신 rerank 융합을 켤 수 있다. 합성 eval에서
   `context_precision` 0.350 → 0.815 회복(위 [평가 결과](#평가-결과--build--measure--diagnose--fix--re-measure) 참조).
